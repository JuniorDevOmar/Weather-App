...
export class WeatherInfo {
  ...
  public getGeocodedInfo(location: string): Observable<City[]> {
    ...
    return this.#httpClient.get<Geocode>(url).pipe(map((res) => res.results || []));
  }
  ...
  public getDailyWeatherInfo(
    latitude: string,
    longitude: string,
  ): Observable<DailyWeatherResponse> {
    const url = this.#dailyWeatherUrl
      .replace('{latitude}', latitude)
      .replace('{longitude}', longitude);
  ...
  public getHourlyWeatherInfo(
    ...
    const url = this.#hourlyWeatherUrl
      .replace('{latitude}', latitude)
      .replace('{longitude}', longitude);
